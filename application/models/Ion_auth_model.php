...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if (substr( $_SERVER['REQUEST_URI'], 0, 6) == '/admin') {
			...
			$time = time();
			$query = $this->db->select("username, email, id, password, 1 as active, '$time'  as last_login", false)
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get('users');
		...
		if ($query->num_rows() === 1)
		{
			...
			if (substr( $_SERVER['REQUEST_URI'], 0, 6) == '/admin') 
				...
				$password = $this->hash_password_db($user->id, $password) == $user->password;
			...
			if ($password === TRUE)
			{
				...
				if (substr( $_SERVER['REQUEST_URI'], 0, 6) == '/admin') {
					...
					// simulate $this->set_session($user);
					$session_data = array(
						'identity'             => $user->email,
	...
	public function users($groups = NULL)
	{
		...
		if (substr( $_SERVER['REQUEST_URI'], 0, 6) == '/admin')
			...
			$this->response = $this->db->get('users');
	...
	public function get_users_groups($id=FALSE)
	{
		...
		if (substr( $_SERVER['REQUEST_URI'], 0, 6) == '/admin') {
			...
			return $this->db->query("SELECT 1 as id, 'members' as name, 'General User' as description");
		}
	}
